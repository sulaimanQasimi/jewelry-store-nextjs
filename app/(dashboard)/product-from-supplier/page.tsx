    ...
    { key: 'createdAt', label: 'تاریخ', render: (r) => formatDate(r.createdAt) },
    {
      ...
      render: (r) => (
        <div className="flex gap-2">
          <button
            type="button"
            onClick={() => router.push(`/product-from-supplier/${r.id}/edit`)}
          ...
          <button
            type="button"
            onClick={() => router.push(`/product-from-supplier/${r.id}`)}
  ...
  return (
    <div className="space-y-8">
      <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="font-heading text-2xl font-semibold text-charcoal dark:text-slate-200">خرید جنس از تمویل‌کننده</h1>
          <p className="mt-1 text-sm text-charcoal-soft dark:text-slate-400">لیست خریدهای جنس از تمویل‌کننده را با فیلتر و صفحه‌بندی مشاهده کنید.</p>
        </div>
        <Link
          href="/product-from-supplier/new"
      ...
      <section>
        <h2 className="font-heading text-lg font-semibold text-charcoal dark:text-slate-200 mb-4">لیست خرید جنس از تمویل‌کننده</h2>
        <FilterBar
          search={search}
        ...
        <div className="mt-4">
          <DataTable<SupplierProduct>
            ...
            keyField="id"
            loading={loading}
            emptyMessage="خرید جنس یافت نشد"
            pagination={{
              page,
              limit,
              total,
