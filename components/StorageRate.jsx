        ...
        <div className="relative">
            ...
            <div className={`absolute transition-all duration-300 ease-in-out z-50 min-w-[18rem] ${isCurrencyToggleOn ? 'top-14 -right-0 opacity-100 visible' : 'top-14 -right-0 opacity-0 invisible pointer-events-none'}`}>
                <div className="card-luxury p-6 rounded-2xl shadow-[0_12px_40px_-8px_rgba(28,28,28,0.15)] border border-gold-200/50 dark:border-slate-600/50">
                    <h2 className="font-heading text-lg font-semibold mb-4 text-charcoal dark:text-white">موجودی دخل</h2>
                    <div className="space-y-4">
                        <div>
                            <label className="block text-sm font-medium text-charcoal dark:text-slate-200 mb-1">دخل دالر</label>
                            <input type="number" step="0.01" value={usdRate} onChange={(e) => setUsdRate(parseFloat(e.target.value) || "")} className="input-luxury w-full" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-charcoal dark:text-slate-200 mb-1">دخل افغانی</label>
