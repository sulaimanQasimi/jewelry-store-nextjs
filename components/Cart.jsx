    ...
    <div className="mt-6">
      <div className="card-luxury overflow-hidden rounded-2xl">
        ...
        {cart.map((product, index) => {
          ...
          const imgSrc = product.image ? (product.image.startsWith('http') ? product.image : `/${product.image}`) : null
          return (
            <ul key={productId ?? index} className="grid grid-cols-[5fr_5fr_2fr_2fr_3fr_4fr_8fr_8fr_2fr] text-center justify-center items-center text-charcoal text-xs border-b border-gold-100 last:border-0 hover:bg-champagne/40 transition-colors">
              ...
              <li className="py-2 px-2 border-l border-gold-100 flex justify-center">
                {imgSrc ? <img className="w-32 h-20 object-cover rounded-lg border border-gold-200" src={imgSrc} alt="" /> : <span className="text-charcoal-soft">—</span>}
              </li>
              ...
              <li className="py-2 flex justify-center">
                <button type="button" className="p-1.5 rounded-[8px] text-rosegold hover:bg-gold-100 transition-all duration-300" onClick={() => removeFromCart(productId)} aria-label="حذف">✕</button>
              </li>
            </ul>
          )
        })}
