...
export default function SupplierFormModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'edit' && initialData?.id) {
        const { data } = await axios.put(`/api/supplier/update/${initialData.id}`, {
          name: form.name.trim(),
        ...
        const { data } = await axios.post('/api/supplier/create', {
          name: form.name.trim(),
          phone: form.phone.trim(),
          address: form.address?.trim() || null
        })
  ...
  const title = mode === 'edit' ? 'ویرایش تمویل‌کننده' : 'افزودن تمویل‌کننده'
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          ...
          {mode === 'edit' && (
            <div className="sm:col-span-2 flex items-center gap-2">
              ...
              />
              <label htmlFor="supplier-isActive" className="text-sm text-charcoal">
                فعال
              </label>
            </div>
          )}
