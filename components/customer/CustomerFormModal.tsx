    ...
    <Modal open={open} onClose={onClose} title={title}>
      <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <FormField label="عکس">
              <div className="flex items-center gap-4">
                <div className="w-24 h-24 rounded-xl border-2 border-gold-200 bg-gold-50/50 flex items-center justify-center overflow-hidden">
                  {imagePreview ? (
                    <img
                      src={imagePreview.startsWith('blob') || imagePreview.startsWith('http') ? imagePreview : (typeof window !== 'undefined' ? window.location.origin : '') + imagePreview}
                      alt=""
        ...
        <FormField label="آدرس">
          <input name="address" className="input-luxury w-full" value={form.address ?? ''} onChange={handleChange} />
        </FormField>
        ...
        <FormField label="یادداشت">
          <textarea name="notes" className="input-luxury w-full min-h-[80px]" value={form.notes ?? ''} onChange={handleChange} />
        </FormField>
        ...
        <div className="border-t border-gold-200 pt-4">
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              <input name="facebookUrl" className="input-luxury flex-1" placeholder="Facebook URL" value={form.facebookUrl ?? ''} onChange={handleChange} />
            </div>
            <div className="flex items-center gap-2">
              <InstagramIcon className="w-5 h-5 text-[#E4405F]" />
              <input name="instagramUrl" className="input-luxury flex-1" placeholder="Instagram URL" value={form.instagramUrl ?? ''} onChange={handleChange} />
            </div>
            <div className="flex items-center gap-2">
              <WhatsAppIcon className="w-5 h-5 text-[#25D366]" />
              <input name="whatsappUrl" className="input-luxury flex-1" placeholder="WhatsApp URL" value={form.whatsappUrl ?? ''} onChange={handleChange} />
            </div>
            <div className="flex items-center gap-2">
              <TelegramIcon className="w-5 h-5 text-[#26A5E4]" />
              <input name="telegramUrl" className="input-luxury flex-1" placeholder="Telegram URL" value={form.telegramUrl ?? ''} onChange={handleChange} />
            </div>
          </div>
        </div>
